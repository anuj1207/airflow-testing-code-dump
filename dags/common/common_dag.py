...
BASE_DAG_ID = "as_op_migration_workflow"
...
def get_dag_args(pipeline_id: str):
    return {
...
def get_tasks(get_path_to_conf):
    (
        PythonOperator(task_id=PIVOT_OLD_DATA_TASK_ID, python_callable=get_path_to_conf)
        >> PythonOperator(task_id=PIVOT_OLD_DATA_TASK_ID+"_2", python_callable=get_path_to_conf)
        # >> DatabricksSubmitRunDeferrableOperator(
